...
(function () {
  var STORAGE_KEY = 'theshed-accent';
  var STORAGE_HEX = 'theshed-accent' + "-hex";
  var CUSTOM_VARS = ["--accent", "--accent-2", "--accent-3", "--accent-deep", "--accent-rgb", "--on-accent"];
  var ACCENTS = { original: '#3dccc7', red: "#e10600", green: "#12b85a", cobalt: "#0047ab", yellow: "#e6b800", orange: "#ff5c00", custom: '#3dccc7' };
  ...
  function syncCustomUi(hex, on) {
    ...
    if (sw) sw.style.background = hex || '#3dccc7';
  ...
  function applyAccent(name) {
    ...
    var meta = document.querySelector('meta[name="theme-color"]');
    if (meta) meta.setAttribute("content", ACCENTS[key] || '#3dccc7');
    document.querySelectorAll("#themeMenu [data-accent]").forEach(function (btn) {
      var on = btn.getAttribute("data-accent") === key;
      btn.classList.toggle("is-active", on);
      btn.setAttribute("aria-checked", on ? "true" : "false");
    });
    syncCustomUi('#3dccc7', false);
  }
